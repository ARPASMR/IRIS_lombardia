...
/*MIN TEMPERATURE LOMBARDIA*/
//Provo tematizzazione con CLUSTER e cerchi colorati - devo definire un cluster strategy per ogni layer altrimenti non so perche sballa e lo fa solo sull'ultimo layer per cui e' definito!
var clust_temp_min_lm = new OpenLayers.Strategy.Cluster({
    distance: 30 //pixel
    , threshold: 2
});      
var style_temp_min_clust_lm = new OpenLayers.Style(
    {   title: "${getTitle}",
        label: "${getLabel}", labelAlign: "cm", fontWeight: "bold", fontFamily: "sans-serif", fontSize: 15
        ,graphicName: "circle", fillColor: "${getFillColor}", fillOpacity: 0.8, strokeColor:"black", pointRadius: "${getRadius}", strokeWidth: "${getWidth}"
    }
    //A quanto pare il context non funzione se c'e una Rule
    ,{context: context_temp_min_lm} //context dentro allo style
);
var styleMap_temp_min_clust_lm = new OpenLayers.StyleMap({
    "default": style_temp_min_clust_lm,
    "select": new OpenLayers.Style({fontSize: 19, pointRadius: 18, fillColor: "blue", fillOpacity: 0.8}),
    "temporary": new OpenLayers.Style({pointRadius: 20, fontSize: 19, cursor: "pointer"})
});
var temperatura_min_lm = new OpenLayers.Layer.Vector(default_layer_name, {
        styleMap: styleMap_temp_min_clust_lm,
        //minScale: 250000,
        strategies: [new OpenLayers.Strategy.Fixed()
            ,new OpenLayers.Strategy.Refresh({force: true, interval:150000})
            //,filterStrategy_temp
		...
		featureNS: "http://www.tinyows.org/",
		//featureType: "v_datistazioni", //vecchia logica
		featureType: "v_last_terma_min", //nuova logica dati da meteo_real_time
		//featurePrefix: "tows",
                readFormat: new OpenLayers.Format.GML({
			'internalProjection': OL_32632,
			'externalProjection': OL_32632
...
temperatura_min_lm.setVisibility(false);
var store_terma_min_lm = new GeoExt.data.FeatureStore({
        fields: [
                {name: "idstazione", type: "integer"},
                {name: "denominazione", type: "string"},
                {name: "idsensore", type: "integer"},
        ...
        layer: temperatura_min_lm
});
store_terma_min_lm.on('load', function(store){
        store.sort('timeultimovalore', 'ASC');
});
var columns_terma_min_lm = new Ext.grid.ColumnModel({
        defaults: {
                sortable: true
        },
        columns: [
...
/*MAX TEMPERATURE LOMBARDIA*/
//Provo tematizzazione con CLUSTER e cerchi colorati - devo definire un cluster strategy per ogni layer altrimenti non so perche sballa e lo fa solo sull'ultimo layer per cui e' definito!
var clust_temp_max_lm = new OpenLayers.Strategy.Cluster({
    distance: 30 //pixel
    , threshold: 2
});      
var style_temp_max_clust_lm = new OpenLayers.Style(
    {   title: "${getTitle}",
        label: "${getLabel}", labelAlign: "cm", fontWeight: "bold", fontFamily: "sans-serif", fontSize: 15
        ,graphicName: "circle", fillColor: "${getFillColor}", fillOpacity: 0.8, strokeColor:"black", pointRadius: "${getRadius}", strokeWidth: "${getWidth}"
    }
    //A quanto pare il context non funzione se c'e una Rule
    ,{context: context_temp_max_lm} //context dentro allo style
);
var styleMap_temp_max_clust_lm = new OpenLayers.StyleMap({
    "default": style_temp_max_clust_lm,
    "select": new OpenLayers.Style({fontSize: 19, pointRadius: 18, fillColor: "blue", fillOpacity: 0.8}),
    "temporary": new OpenLayers.Style({pointRadius: 20, fontSize: 19, cursor: "pointer"})
});
var temperatura_max_lm = new OpenLayers.Layer.Vector(default_layer_name, {
        styleMap: styleMap_temp_max_clust_lm,
        //minScale: 250000,
        strategies: [new OpenLayers.Strategy.Fixed()
            ,new OpenLayers.Strategy.Refresh({force: true, interval:150000})
            //,filterStrategy_temp
		...
		featureNS: "http://www.tinyows.org/",
		//featureType: "v_datistazioni", //vecchia logica
		featureType: "v_last_terma_max", //nuova logica dati da meteo_real_time
		//featurePrefix: "tows",
                readFormat: new OpenLayers.Format.GML({
			'internalProjection': OL_32632,
			'externalProjection': OL_32632
...
temperatura_max_lm.setVisibility(false);
var store_terma_max_lm = new GeoExt.data.FeatureStore({
        fields: [
                {name: "idstazione", type: "integer"},
                {name: "denominazione", type: "string"},
                {name: "idsensore", type: "integer"},
        ...
        layer: temperatura_max_lm
});
store_terma_max_lm.on('load', function(store){
        store.sort('timeultimovalore', 'ASC');
});
var columns_terma_max_lm = new Ext.grid.ColumnModel({
        defaults: {
                sortable: true
        },
        columns: [
